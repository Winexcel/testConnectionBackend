...
class BaseController {
  ...
  handle(req, res, method) {
    ...
    this.beforeValidation(req, res, method, params, response);
    const validateResult = validateParams({ ...params }, this.validations[method]);
    this.afterValidation(req, res, method, params, validateResult, response).then((next) => {
      if (next) {
        if (validateResult.isValid) {
          this.model.handle(req, res, method, { ...params },
            resHandler.callback.bind(resHandler, req, res));
        } else {
